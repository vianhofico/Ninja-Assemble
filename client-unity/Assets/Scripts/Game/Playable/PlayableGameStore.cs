...
namespace NinjaAssemble.Playable
{
    public sealed class PlayableGameStore
    {
        ...
        {
            PlayerDto player = await api.LoginGuestAsync(guestKey, displayName); PlayerId = player.id;
        ...
        {
            QuestClaimDto result = await api.ClaimQuestAsync(PlayerId, questId);
        ...
        {
            MailClaimDto result = await api.ClaimMailAsync(PlayerId, mailId);
            if (!result.replayed)
            {
                foreach (MailGrantDto grant in result.grants ?? Array.Empty<MailGrantDto>())
                {
                    if (!string.Equals(grant.kind, "CURRENCY", StringComparison.OrdinalIgnoreCase)) continue;
