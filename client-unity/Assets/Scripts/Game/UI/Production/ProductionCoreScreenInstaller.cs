...
{
    public static class ProductionCoreScreenInstaller
    {
        ...
        {
            CoreScreenCatalog.Spec spec=CoreScreenCatalog.Resolve(scene.name);if(spec==null)return;
            GameObject safeObject=GameObject.Find("SafeArea");if(safeObject==null)return;Transform safe=safeObject.transform;
            if(safe.Find("ProductionCorePanel")!=null)return;
            Transform legacy=safe.Find("BodyPanel");if(legacy==null)return;
            TMP_Text sourceBody=legacy.Find("Body")?.GetComponent<TMP_Text>();TMP_Text sourceStatus=legacy.Find("Status")?.GetComponent<TMP_Text>();
            Transform sourceActionTransform=legacy.Find("PrimaryAction");Button sourceButton=sourceActionTransform?.GetComponent<Button>();TMP_Text sourceAction=sourceActionTransform?.Find("Label")?.GetComponent<TMP_Text>();

            Image panel=ProductionUiFactory.Panel(safe,"ProductionCorePanel",ProductionUiTokens.Surface);RectTransform panelRect=panel.rectTransform;ProductionUiFactory.Anchor(panelRect,new Vector2(.025f,.17f),new Vector2(.975f,.88f));
            ...
            TMP_Text body=ProductionUiFactory.Text(card.transform,"LiveBody","Loading live state...",24f,TextAlignmentOptions.TopLeft,ProductionUiTokens.Text);RectTransform bodyRect=body.rectTransform;ProductionUiFactory.Anchor(bodyRect,new Vector2(.035f,.20f),new Vector2(.965f,.94f));
