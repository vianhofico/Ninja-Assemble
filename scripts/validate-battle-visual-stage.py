...
def main() -> int:
    ...
    for path in REQUIRED_FILES:
        if not path.exists(): errors.append(f"missing runtime battle file: {path.relative_to(ROOT)}")
    ...
    for token in ("BattleParticipantDto", "participants", "battleUnitId", "characterId", "variant", "side", "slot", "maxHp", "timestampMs", "rageAfter", "durationMs"):
        if token not in dto: errors.append(f"PlayableDtos missing participant/realtime token: {token}")
    ...
    try: runtime = json.loads(RUNTIME_CATALOG.read_text(encoding="utf-8"))
        ...
        errors.append(f"invalid runtime art catalog JSON: {exc}"); runtime = {"entries": []}
    actual = {(entry.get("characterId", "").strip(), entry.get("variant", "").strip()): entry for entry in runtime.get("entries", [])}
    expected = {(row["character_id"].strip(), row["variant"].strip()): row for row in manifest_rows}
    if set(actual) != set(expected): errors.append(f"runtime art catalog identities differ from manifest: expected={len(expected)} actual={len(actual)}")
    field_map = {"portraitAddress":"portrait_address","iconAddress":"icon_address","prefabAddress":"prefab_address","animationSet":"animation_set","vfxSet":"vfx_set","sfxSet":"sfx_set","status":"status"}
    for key in set(actual) & set(expected):
        for json_field, csv_field in field_map.items():
            actual_value = str(actual[key].get(json_field, "")).strip(); expected_value = str(expected[key].get(csv_field, "")).strip()
            if json_field == "status": actual_value, expected_value = actual_value.upper(), expected_value.upper()
            if actual_value != expected_value: errors.append(f"runtime art catalog mismatch {key} {json_field}: expected={expected_value!r} actual={actual_value!r}")
    if errors:
