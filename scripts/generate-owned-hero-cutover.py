...
def main() -> int:
    ...
    # Compatibility entries for null/BASE legacy selection.
    ...
    for row in variants:
        ...
        if not hero_id:
            raise SystemExit(f"cannot resolve legacy variant to Hero Version: {character_id}/{variant}")
    ...
    unknown = {str(r["hero_version_id"]) for r in rows} - hero_ids
    if unknown:
    ...
    print(f"OWNED_HERO_CUTOVER_MAP_OK source_variants={len(source_keys)} mapping_rows={len(rows)} hero_versions={len(hero_ids)}")
    return 0
...
def render_sql(rows: list[dict[str, object]]) -> str:
    lines = [
        ...
        "    hero_version_id varchar(128) not null references hero_versions(hero_id),",
    ...
    for row in rows:
        vals = [row[k] for k in ["legacy_character_id", "legacy_variant_id", "hero_version_id"]]
    ...
    lines += [
        ...
        "",
        "-- Backfill each existing ownership row from its selected legacy variant, falling back to the approved primary Hero Version.",
        ...
        "    where u.player_hero_id = ph.id and map.hero_version_id = ph.hero_version_id and map.awakened = true",
        ");",
        "",
        ...
        "    join legacy_variant_hero_version_map map",
        "      on map.legacy_character_id = ph.hero_definition_id and map.legacy_variant_id = u.variant_id",
        "    where map.hero_version_id <> ph.hero_version_id",
        ...
        "        raise exception 'M43 cutover left player_heroes without hero_version_id';",
        "    end if;",
        "end $$;",
        "",
