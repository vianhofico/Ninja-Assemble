...
/** Canonical request for continuous-time deterministic combat. */
public record RealtimeBattleRequest(
        long seed,
        RealtimeBattleRuleset ruleset,
        List<BattleUnitSeed> units
) {
    public RealtimeBattleRequest {
        if (ruleset == null || units == null || units.isEmpty()) {
            throw new IllegalArgumentException("realtime ruleset and units required");
        }
